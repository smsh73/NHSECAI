...
# 워크플로우 엔진 테스트
run_test "워크플로우 엔진 - 세션 생성" "curl -s -X POST $BASE_URL/api/workflows/sessions -H 'Content-Type: application/json' -d '{\"workflowId\":\"test\",\"sessionName\":\"Unit Test Session\"}' | grep -q 'sessionId'"

# 프롬프트 관리자 테스트
run_test "프롬프트 관리자 - 프롬프트 테스트" "curl -s -X POST $BASE_URL/api/prompts/test -H 'Content-Type: application/json' -d '{\"promptId\":\"test\",\"inputData\":{\"news_content\":\"테스트 뉴스\"}}' | grep -q 'success'"

# API 관리자 테스트
run_test "API 관리자 - API 테스트" "curl -s -X POST $BASE_URL/api/api-calls/test -H 'Content-Type: application/json' -d '{\"apiCallId\":\"test\",\"inputData\":{\"symbol\":\"AAPL\"}}' | grep -q 'success'"

# PostgreSQL 연결 테스트
run_test "PostgreSQL 연결" "curl -s $BASE_URL/api/system/status | grep -q 'system'"

# JSON 스키마 검증 테스트
run_test "JSON 스키마 검증" "curl -s $BASE_URL/api/prompts | grep -q 'input_schema'"
