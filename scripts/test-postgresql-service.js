#!/usr/bin/env node
/**
 * PostgreSQL 서비스 executeQuery 기능 통합 테스트
 */
...
class PostgreSQLServiceTest {
  ...
  async initialize() {
    ...
    if (isSQLite) {
      console.error('❌ PostgreSQL을 사용해야 합니다. DATABASE_URL을 PostgreSQL 형식으로 설정하세요.');
      return false;
    }
  ...
  async executeQuery(sql, params = []) {
    if (!this.pool) {
      throw new Error('Pool이 초기화되지 않았습니다');
    }
  ...
  async testSimpleQuery() {
    ...
    try {
      const result = await this.executeQuery('SELECT 1 as test_value, NOW() as current_time');
  ...
  async testWorkflowTableQuery() {
    ...
    try {
      const result = await this.executeQuery('SELECT COUNT(*) as count FROM workflows');
      ...
      this.results.push({
        ...
        success: true,
        duration: Date.now() - startTime,
        details: { warning: error.message }
      });
      console.log(`⚠️ 워크플로우 테이블 조회: ${error.message} (테이블이 없을 수 있음)\n`);
    }
  }
  ...
  async testErrorHandling() {
    ...
    try {
      await this.executeQuery('SELECT * FROM non_existent_table_xyz');
      this.results.push({
        name: '에러 처리',
        success: false,
        duration: Date.now() - startTime,
...
// 테스트 실행
const test = new PostgreSQLServiceTest();
