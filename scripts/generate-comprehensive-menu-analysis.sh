#!/bin/bash
# MENU_SOURCE_CODE_MAPPING.md 기반 종합 분석 문서 생성

set -e

TODAY=$(date +%Y-%m-%d)
OUTPUT_FILE="docs/${TODAY}_menu-comprehensive-analysis.md"
SOURCE_FILE="MENU_SOURCE_CODE_MAPPING.md"
...
# MENU_SOURCE_CODE_MAPPING.md를 읽어서 요약 생성
...
- API 엔드포인트
- 데이터베이스 스키마
- 데이터 소스 (PostgreSQL, Databricks, Azure 서비스 등)
- 화면 구성 요소
- 주요 함수 및 구현 상태
- 기능 완성도

**참고**: 이 문서는 \`MENU_SOURCE_CODE_MAPPING.md\`와 \`FUNCTION_LIST_AND_COMPLETION_RATE.md\`를 기반으로 생성되었습니다.
